...
Token::Token(std::string value, int precedence, TokenType type) : value(value), precedence(precedence), type(type)
{
	switch (type)
	{
	case boolean:
...
TokenMatcher::TokenMatcher(std::regex match, int precedence, TokenType type) : match(match), precedence(precedence), type(type)
{
}
